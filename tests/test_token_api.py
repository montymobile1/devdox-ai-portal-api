...
class TestEndpointSecurity:
    ...
    def test_token_encryption_on_create(
        self, client, mock_encryption_helper, mock_user_model
    ):
    ...
    def test_token_masking_in_response(self, client, mock_encryption_helper):
        ...
        with patch("app.routes.git_tokens.GitLabel") as mock_git_label:
            ...
            data = response.json()
            assert data["data"]["items"][0]["masked_token"] == mask_token(
                "encrypted_token"
            )
...
class TestInputValidation:
    ...
    def test_whitespace_token_handling(
        self, client, mock_encryption_helper, mock_user_model
    ):
