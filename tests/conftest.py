...
TOKEN_ENCRYPTED_1 = (
    "gAAAAABoMFiNIvAc7WIFnoKXBjkpAVrdiTFrhlmZtG8BBwvmy1dtvfEFmupm0fcvDUo3unosoAQz5eclP2QFMnPMLG4Hj21MBt-xTdWL661JnWP-wQarnLI="
)
TOKEN_ENCRYPTED_2 = (
    "gAAAAABoMFiNIvAc7WIFnoKXBjkpAVrdiTFrhlmZtG8BBwvmy1dtvfEFmupm0fcvDUo3unosoAQz5eclP2QFMnPMLG4Hj21MBt-xTdWL661JnWP-wQarnLI="
)
...
def mock_supabase_filter(mock_supabase, token_data_list):
    ...
    async def side_effect_filter(table, filters=None, columns=None, limit=None, single=False):
        if not filters:
            return token_data_list if not single else (token_data_list[0] if token_data_list else None)

        # Filter by label
        if "label" in filters:
            label = filters["label"]
            filtered = [token for token in token_data_list if token.get("label") == label]
...
def mock_supabase_invalid_data(mock_supabase, invalid_token_data_list):
    ...
    async def side_effect_filter(table, filters=None, columns=None, limit=None, single=False):
        if filters and "label" in filters:
            ...
            return [invalid_token_data_list[0]] if not single else invalid_token_data_list[0]
        return invalid_token_data_list if not single else (invalid_token_data_list[0] if invalid_token_data_list else None)
...
def mock_api_response():
    ...
    with patch("app.routes.git_tokens.APIResponse") as mock:
        ...
        # Set up error method
        mock.error = MagicMock(return_value={"success": False, "message": "Operation failed", "data": None})
