...
class TestGetReposRoute:
	...
	def test_empty_repo_list(self, monkeypatch, client, mock_authenticated_user):
		monkeypatch.setattr(
			...
			lambda *a, **k: self._build_mock_repo_query()
		)
	...
	def test_created_at_ordering(self, monkeypatch, client, mock_authenticated_user):
		repo_docs = [
			dict(MOCK_REPO_DOCUMENT, id=i,
			     created_at=(datetime.datetime(2025, 1, 1) - datetime.timedelta(days=i)).isoformat())
			for i in range(3)
		]
		monkeypatch.setattr(
			"app.models.Repo.filter",
	...
	@pytest.mark.parametrize("input_header", [
		None,
		"Bearer",
		"BearerInvalid",
		"Token faketoken123"
