...
class TestGetReposRoute:
    ...
    def test_empty_repo_list(self, monkeypatch, client, mock_authenticated_user):
        monkeypatch.setattr(
            "app.models.Repo.filter", lambda *a, **k: self._build_mock_repo_query()
        )
    ...
    def test_single_repo(self, monkeypatch, client, mock_authenticated_user):
        monkeypatch.setattr(
            "app.models.Repo.filter",
            lambda *a, **k: self._build_mock_repo_query(1, [MOCK_REPO_DOCUMENT]),
        )
        response = client.get(self.endpoint, headers=self.headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_multiple_repos(self, monkeypatch, client, mock_authenticated_user):
        ...
        monkeypatch.setattr(
            "app.models.Repo.filter",
            lambda *a, **k: self._build_mock_repo_query(5, repo_docs),
        )
        response = client.get(self.endpoint, headers=self.headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_extreme_pagination(self, monkeypatch, client, mock_authenticated_user):
        ...
        monkeypatch.setattr(
            "app.models.Repo.filter",
            lambda *a, **k: self._build_mock_repo_query(1000, repo_docs),
        )
        response = client.get(
            f"{self.endpoint}?limit=1000&offset=0", headers=self.headers
        )
    ...
    def test_created_at_ordering(self, monkeypatch, client, mock_authenticated_user):
        ...
        monkeypatch.setattr(
            "app.models.Repo.filter",
            lambda *a, **k: self._build_mock_repo_query(3, repo_docs),
        )
        response = client.get(self.endpoint, headers=self.headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_handles_database_error(
        self, monkeypatch, client_permissive, mock_authenticated_user
    ):

        expected_exception = Exception("DB failure")

        monkeypatch.setattr(
            Repo, "filter", lambda *a, **kw: (_ for _ in ()).throw(expected_exception)
        )
    ...
    def test_all_returns_non_list(
        self, monkeypatch, client_permissive, mock_authenticated_user
    ):
        monkeypatch.setattr(
            Repo,
            "filter",
            lambda *a, **kw: self._build_mock_repo_query(1, "not-a-list"),
        )
        response = client_permissive.get(self.endpoint, headers=self.headers)
        assert response.status_code == generic_exception_handler_status_code
    ...
    def test_broken_query_chain(
        self, monkeypatch, client_permissive, mock_authenticated_user
    ):
        mock_query = self._build_mock_repo_query(
            2, [MOCK_REPO_DOCUMENT], simulate_partial_chain=True
        )
    ...
    def test_invalid_repo_schema(
        ...
        monkeypatch.setattr(
            Repo, "filter", lambda *a, **k: self._build_mock_repo_query(1, [bad_doc])
        )
        response = client_permissive.get(self.endpoint, headers=self.headers)
        assert response.status_code == generic_exception_handler_status_code
    ...
    @pytest.mark.parametrize(
        "input_header", [None, "Bearer", "BearerInvalid", "Token faketoken123"]
    )
    def test_invalid_authorization_header(self, monkeypatch, client, input_header):
        ...
        json_data = response.json()
        assert (
            json_data["debug"]["exception"].lower()
            == UnauthorizedAccess.__name__.lower()
        )
