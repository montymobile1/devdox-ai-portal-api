...
def override_revoke_service_success():
    ...
    store.stored_keys.append(SimpleNamespace(id=fake_key_id, user_id="user123", is_active=True))
...
class TestRevokeApiKeyRouter:
    ...
    def test_successful_revoke(self, client, override_auth_user, override_revoke_service_success):
    ...
    def test_revoke_not_found(self, client, override_auth_user, override_revoke_service_not_found):
        response = client.delete(f"{self.route_url}{uuid.uuid4()}")
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_revoke_unauthorized(self, client, override_auth_user_unauth, override_revoke_service_success):
        _, fake_key_id = override_revoke_service_success
        response = client.delete(f"{self.route_url}{fake_key_id}")
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_invalid_uuid_path(self, client, override_auth_user, override_revoke_service_success):
