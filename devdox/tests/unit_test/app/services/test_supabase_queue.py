...
class TestSupabaseQueueFailJob:
    """Test SupabaseQueue fail_job functionality"""
    ...
    @pytest.mark.asyncio
    async def test_fail_job_with_retry(self, mock_queue):
        ...
        result = await queue.fail_job(
            job_data, error="Processing failed", error_trace="Traceback...", retry=True
        )

        assert result is True
        # Should delete original message and send delayed retry
        assert len(mock_queue_instance.deleted_messages) == 1
        assert len(mock_queue_instance.sent_messages) == 1

        retry_msg = mock_queue_instance.sent_messages[0]
        assert retry_msg["delay"] > 0  # Should have retry delay

    @pytest.mark.asyncio
    async def test_fail_job_max_attempts_reached(self, mock_queue):
        ...
        result = await queue.fail_job(job_data, error="Final failure", retry=True)

        assert result is True
        # Should archive the job
        assert len(mock_queue_instance.archived_messages) == 1
        archived_msg = mock_queue_instance.archived_messages[0]
        assert archived_msg["msg_id"] == 42

    @pytest.mark.asyncio
    async def test_fail_job_no_retry(self, mock_queue):
        ...
        result = await queue.fail_job(job_data, error="Critical error", retry=False)

        assert result is True
        # Should archive immediately
        assert len(mock_queue_instance.archived_messages) == 1
        assert len(mock_queue_instance.sent_messages) == 0  # No retry

    @pytest.mark.asyncio
    async def test_fail_job_missing_msg_id(self, mock_queue):
        """Test job failure with missing message ID"""
        queue, mock_queue_instance = mock_queue
        ...
        result = await queue.fail_job(job_data, error="Error")

        assert result is False
        assert len(mock_queue_instance.archived_messages) == 0

    @pytest.mark.asyncio
    async def test_fail_job_with_json_payload(self, mock_queue):
        """Test job failure with JSON string payload"""
        queue, mock_queue_instance = mock_queue
        ...
        job_data = {
            ...
            "queue_name": "processing",
            "attempts": 1,
            "payload": json.dumps({"repo_id": "repo-456"}),  # JSON string
        }

        result = await queue.fail_job(job_data, error="Error", retry=True)
...
class TestSupabaseQueueIntegration:
    ...
    @pytest.mark.asyncio
    async def test_job_failure_and_retry_lifecycle(self):
        """Test job failure and retry lifecycle"""
        with patch("app.services.supabase_queue.PGMQueue") as mock_pgmqueue_class:
            mock_queue_instance = MockPGMQueue()
            mock_pgmqueue_class.return_value = mock_queue_instance
            ...
            # Enqueue job
            job_id = await queue.enqueue("processing", {"repo_id": "repo-123"})
            ...
            job_data = {
                ...
                "attempts": 1,
                "max_attempts": 3,
                "payload": {"repo_id": "repo-123"},
            }

            result = await queue.fail_job(job_data, "Processing error", retry=True)

            assert result is True
            assert len(mock_queue_instance.sent_messages) == 2  # Original + retry
            assert len(mock_queue_instance.deleted_messages) == 1  # Original deleted
