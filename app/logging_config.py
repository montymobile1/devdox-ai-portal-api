...
from app.middlewares.trace_id_middleware import TraceIDLogFilter
...
def setup_logging():
    ...
    trace_id_log_filter = TraceIDLogFilter()

    # Console handler for logging to console
    console_handler = logging.StreamHandler()
    console_handler.setFormatter(formatter)
    console_handler.addFilter(trace_id_log_filter)
    logger.addHandler(console_handler)

    # File handler with rotating log (max 5MB per file, keeping 3 backup files)
    file_handler = RotatingFileHandler('app.log', maxBytes=5 * 1024 * 1024, backupCount=3)
    file_handler.setFormatter(formatter)
    file_handler.addFilter(trace_id_log_filter)
