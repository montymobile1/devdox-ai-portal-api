...
def build_repo_dict(repo_info: Dict[str, Any], platform: str) -> Dict[str, Any]:
    ...
    common_fields = {
        ...
        "stargazers_count": repo_info.get("stargazers_count") or repo_info.get("star_count", 0),
    }
...
def fetch_gitlab_repos(access_token: str, pagination: PaginationParams) -> Tuple[List[Dict[str, Any]], int]:
    ...
    return [build_repo_dict(repo, GitHosting.GITLAB) for repo in raw_repos.get("repositories", [])], raw_repos.get(
        "pagination_info", {}
    ).get("total_count", len(raw_repos.get("repositories", [])))


def fetch_github_repos(access_token: str, pagination: PaginationParams) -> Tuple[List[Dict[str, Any]], int]:
    """Fetch repositories from GitHub."""
    github = GitHubManager(access_token=access_token)
    result = github.get_user_repositories(page=pagination.offset + 1, per_page=pagination.limit)

    repos = [build_repo_dict(repo, GitHosting.GITHUB) for repo in result.get("repositories", [])]
...
async def get_repos(
    ...
    repos = await query.order_by("-created_at").offset(pagination.offset).limit(pagination.limit).all()
...
async def get_repos_from_git(
    ...
    try:
        ...
        return APIResponse.error(message=constants.TOKEN_NOT_FOUND, status_code=status.HTTP_404_NOT_FOUND)
